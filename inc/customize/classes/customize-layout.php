		...
		<div class="customize-control-<?php echo esc_html( $this->type ); ?> ">
			...
			<?php foreach ( $this->choices as $value => $label ) { ?>
				...
				<label <?php echo( $value == $selected ) ? 'class="selected"' : ''; ?> >
					...
					<input type="radio" name="bizi_<?php echo esc_attr( $this->type ); ?>" value="<?php echo esc_attr( $value ); ?>" data-id="<?php echo esc_attr($this->id); ?>"
						<?php echo ( $selected == $value ) ? 'checked="checked"' : '';?>
					>

					<?php if ( $label == 'none' ) { ?>

                    <img src="<?php echo esc_url(bizi_PLUGIN_URL. '/assets/images/right.png'); ?>" alt="layout" />
