...
add_action('widgets_init', 'bizi_information');

function bizi_information()
{
...
class information_widget extends WP_Widget
{
    ...
    {
        /* Widget settings. */
        $widget_ops = array('classname' => 'information_widget', 'description' => esc_html__('Easy add image for sidebar.', 'bizi'));

        /* Widget control settings. */
        $control_ops = array('width' => 250, 'height' => 350, 'id_base' => 'information_widget');

        /* Create the widget. */
        parent::__construct('information_widget', esc_html__('+NA: Image', 'bizi'), $widget_ops, $control_ops);
    }
    ...
    function widget($args, $instance)
    {
        ...
        ?>
        <div class="bizi-image-content">
            <?php if ($image) {
                ?>
                <a class="clearfix" href="<?php echo esc_url($link); ?>">
                    <img class="bizi-image" src="<?php echo esc_url($image)?>" alt="img" />
                </a>
            <?php } ?>
        </div>
        <?php
    ...
    function form($instance)
    {
        ...
        <p id="<?php echo esc_attr($this->get_field_id('image').'-wrapp'); ?>">
            <label for="<?php echo esc_attr($this->get_field_id('image')); ?>"><?php esc_html_e('Image:', 'bizi'); ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('link')); ?>"><?php echo esc_html_e('Url:', 'bizi'); ?></label>
